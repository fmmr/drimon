...
Every ThingSpeak entry carries e.g. `T-OK_SHADE_W-OPEN_B-OK_P-HIGH_WF-HIT_BS-abc123_WT-388_FC-0_BV-4.09_TU-5435_SD-0`. Prefixes: `T-` temp class, (bare) light level, `W-` window, `B-` battery, `P-` pressure, `WF-` WiFi cache outcome (`HIT`/`MISS`/`FBK`/`FAIL`), `BS-` last 3 bytes of AP BSSID (mesh-node ID), `WT-` connect time (ms), `FC-` failed-wake streak (RTC counter, reset if any channel POST succeeds, saturates at 65535), `BV-` battery voltage (V, 2 dp), `TU-` wake-to-end-of-measure time (ms), `SD-` display-pause (0 or 12000). Full decoder: `documentation/QUICK_REFERENCE.md`.
