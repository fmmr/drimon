...
- `raw-statuses.html` — flat two-column dump (time · raw status string) of merged wakes across all 3 status channels, sorted newest-first. Click any row to copy the status string to clipboard. Files: `js/raw-statuses.js` + `css/raw-statuses.css`. URL param: `n` (1–8000, default 100). Same merge logic as status.html (identical status within 60 s = one row). Useful for grabbing raw strings for jq/curl analysis or for eyeballing recent activity without the interpretive UI.
