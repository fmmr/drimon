...
| 2626867 | `EXT_CHANNEL` | MET temp/hum, temp/hum diffs, wind, precip, apparent temp, UV | MATLAB scraper |
...
- `status.html` — single unified page for all status inspection. Parses the ThingSpeak `status` field (`WF-`, `BS-`, `WT-`, `FC-`, `T-`, `W-`, `B-`, `P-`, light token) plus TECH_CHANNEL fields 2 & 4 (battery V, wake duration): WiFi HIT/FBK/MISS/FAIL stacked bars per day (width = post count), WT p50/p95 per day, battery V min–max per day, TU p50/p95 per day, FC per day, class-distribution mini-charts, and a last-N statuses table. Two independent switches control layout: `showOnlyStatus=true` skips all charts/per-day breakdowns/distributions and shows only the table; `raw=true` renders the table as raw status strings (3 cols: time / channels / raw) instead of the parsed 20-column projection. All four combinations valid. Files: `js/status.js` + `js/status-recent.js` (shared parser + both table renderers) + `css/status.css`. URL params: `days` (1–400, default 14), `results` (max 8000), `n` (rows in the table, 1–8000, default 150), `showOnlyStatus`, `raw` (both default false). Fetches `/status.json` from all 3 status channels (2568299, 2584548, 2584547) and merges by identical status string within a 60 s window — same wake, one row. Presets bar at the top links common combinations (standard, 7d, 30d, kun statuser, rå, …). Note: WF-/WT-/SD-/BS-/FC- tokens were added by firmware updates; older entries carry T-/W-/B-/P-/light only, so those charts have a shorter effective window than the distributions.
- `raw-statuses.html` — thin meta-refresh redirect to `status.html?raw=true&showOnlyStatus=true`. Kept for old bookmarks; new usage should go through the presets on `status.html`.
...
Every ThingSpeak entry carries e.g. `T-OK_SHADE_W-OPEN_B-OK_P-HIGH_WF-HIT_BS-14918294f9c4_WT-388_FC-0_PF-0_BV-4.09_TU-5435_LR-200.200.200_PR-0.0.0_LTU-5312_LP-3120_LT-11250_WR-8.4_LS-42.OK_SD-0`. Prefixes: `T-` temp class, (bare) light level, `W-` window, `B-` battery, `P-` pressure, `WF-` WiFi cache outcome (`HIT`/`MISS`/`FBK`/`FAIL`), `BS-` full 6-byte BSSID as hex (mesh-node radio MAC, LAN MAC +1/+2 typically), `WT-` connect time (ms), `FC-` WiFi-fail streak (RTC counter, reset if any channel POST succeeds), `PF-` post-fail streak (WiFi OK but all 3 POSTs failed, RTC counter, reset if any channel POST succeeds), `BV-` battery voltage (V, 2 dp), `TU-` wake-to-end-of-measure time (ms), `LR-` previous wake's 3 HTTP result codes (dot-sep, e.g. `200.429.0`), `PR-` previous wake's 3 per-channel retry counts (dot-sep, `0.0.0` = clean, up to `MAX_POST_RETRY` per channel), `LTU-` previous wake's measure time (ms, mirror of that wake's TU), `LP-` previous wake's post-cycle time (ms), `LT-` previous wake's total wall-clock time (ms, setup→sleep), `WR-` wake reason as `<resetReason>.<wakeupCause>` (e.g. `8.4` = normal deep-sleep timer wake, `1.0` = power-on cold boot); anything but `8.*` = a cold-path reset that wiped RTC. `LS-` previous wake's serial + last stage (NVS-stored, survives panic) as `<wakeNum>.<stage>`: stage `OK` = clean deep sleep, else where the code died — `SP` setupPins, `ID` initDisplays, `WF` connectToWiFi, `IS` initSensors, `MS` measure, `PS` post, `DP` display-pause; `??` = first-ever boot (NVS empty). Serial is a monotone uint16 (wraps at 65535) that makes every wake's status unique so identical panic-loop rows don't merge. `SD-` display-pause (0 or 8000). `L*` prefix marks tokens carrying previous-wake data. Full decoder: `documentation/QUICK_REFERENCE.md`.
