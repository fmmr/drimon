...
Every power-domain interface gets its own labelled, keyed connector on the PCB. All routing between them is copper trace, not hand-soldered wire. This is a hard rule — v1.2 has repeatedly lost inter-module hand-soldered joints (see `documentation/HARDWARE_SOLAR_CHARGING_CHECK.md` for the earlier solar-to-charger incident; a separate joint on the BAT+/BAT− path between charger and battery/gauge failed 2026-08-12). The typical failure mode: joint works under trickle-charge current (small), collapses under WiFi TX burst current (large) → boost output drops → ESP32 brownouts → panic loop until sunrise restores solar-direct power.
