...
# Same progress-friendly filter as compile.sh, plus upload-phase patterns (esptool output:
# Uploading/Connecting/Chip/Writing/Wrote/Hash/Hard resetting/Auto-detected/Serial port/MAC/esptool)
# so flash progress is visible without dumping the full -v firehose.
arduino-cli compile --upload -v --fqbn "$FQBN" --port "$PORT" "$@" . 2>&1 | grep -E '^(Compiling|Linking|Using|Sketch uses|Global variables|Detecting|In file included|Uploading|Connecting|Chip|Writing|Wrote|Hash|Hard resetting|Auto-detected|Serial port|MAC|esptool|Leaving)|(error|warning|note):|^\s+[0-9]+ \||^\s+\|'
