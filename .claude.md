...
### Light Chart "Forskøvet" Bug (June 2025)
**Problem**: Light chart shows two sensors with offset day/night cycles instead of overlapping patterns. Both sensors measure same sunlight but appear shifted in time.

**Root Cause**: Project has two chart systems:
- **Chart Factory** (newer): Uses proper Chart.js time scales with `{x: Date, y: value}` format
- **Chart Renderer** (legacy): Uses category scales with string labels, forcing manual timestamp mapping

**Current State**: Light chart uses Chart Renderer (wrong system), causing:
- Only first series timestamps (130 feeds) used for x-axis
- Second series values (167 feeds) mapped to wrong timestamps
- Manual timestamp synchronization instead of letting Chart.js handle scattered time data

**Solution**: Switch light chart from `createOrUpdateChart()` to Chart Factory time-scale system.

**Files**: 
- `/docs/js/chart-renderer.js` (category scales - legacy)
- `/docs/js/chart-factory.js` (time scales - proper)
- `/docs/js/chart-config.js` (light chart config: `disableSyncTimestamps: true`)