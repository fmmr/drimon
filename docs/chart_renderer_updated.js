...
// Function to position tooltip in relation to chart containers
function positionTooltip(event, chartContainer) {
    const tooltip = d3.select('#global-chart-tooltip');
    const containerBounds = chartContainer.getBoundingClientRect();
    ...
    // Calculate tooltip position to keep it within the chart container
    let tooltipX = mouseX + 15;
    let tooltipY = mouseY - 10;
    
    // Check if tooltip would go outside the chart container on the right
    const tooltipWidth = tooltip.node().offsetWidth || 150; // Default width if not yet rendered
    if (tooltipX + tooltipWidth > containerBounds.right) {
        tooltipX = mouseX - tooltipWidth - 15;
    }
    
    // Check if tooltip would go outside the chart container on the bottom
    const tooltipHeight = tooltip.node().offsetHeight || 80; // Default height if not yet rendered
    if (tooltipY + tooltipHeight > containerBounds.bottom) {
        tooltipY = mouseY - tooltipHeight - 15;
    }
    
    // Make sure tooltip doesn't go outside viewport
    tooltipX = Math.max(5, Math.min(bodyBounds.width - tooltipWidth - 5, tooltipX));
    tooltipY = Math.max(5, Math.min(bodyBounds.height - tooltipHeight - 5, tooltipY));
...
async function drawChart(config, startDate, endDate, results = 8000) {
    ...
    try {
        ...
        // Add active area for mouse interaction even for dense data
        g.append("rect")
            .attr("width", chartWidth)
            .attr("height", chartHeight)
            .attr("fill", "none")
            ...
            .on("mousemove", function(event) {
                ...
                const pos = positionTooltip(event, container);
