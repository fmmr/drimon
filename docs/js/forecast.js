...
const FORECAST_CACHE_TTL = 1 * 60 * 1000; // 20 minute cache (reduced from 30 minutes)
...
async function fetchForecastData() {
    ...
    try {
        ...
        try {
            ...
            logForecast('Successfully fetched forecast data via: ' + result.source);
        } catch (err) {
            // If all methods fail, propagate the error
            logForecast('All forecast fetch attempts failed', err);
            throw err;
        }
